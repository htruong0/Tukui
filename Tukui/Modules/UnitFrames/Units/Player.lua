	...
	if (C.UnitFrames.TotemBar) then
		...
		if C.UnitFrames.TotemBarStyle.Value == "On Screen" then
			...
			for i = 1, MAX_TOTEMS do
				...
				Bar[i].Backdrop.BackgroundTexture = Bar[i].Backdrop:CreateTexture(nil, "OVERLAY", nil, 1)
				...
				Bar[i].Icon = Bar[i]:CreateTexture(nil, "BORDER", nil, 7)
